...
client_id = 945205395926966332LL
...
local function main()
  ...
  elseif pausedFC then
    presence["state"] = ""
    presence["small_image"] = "player_buffer"
    ...
    if o.loop_info == "yes" then
      ...
      if loopFile ~= "" then
        if loopPlaylist ~= "" then
		  ...
		  presence["small_image"] = "player_loop"
        else
          loop = loopFile
		  presence["small_image"] = "player_loop"
        end
  ...
  if file_path ~= nil then
    ...
    if chapter_title ~= nil
      and (string.match(file_path, "www.youtube.com/watch%?v=([a-zA-Z0-9-_]+)&?.*$") ~= nil
        ...
        presence["large_text"] = "Chapter: "..chapter_title
